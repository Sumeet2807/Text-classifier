...
import numpy as np
import re
from utils import BOW_ensemble



class Linear_BOW_ensemble():
    def __init__(self,vectorizer,scale=True,estimator_grps=10,cv_folds=10,lr_penalty = 'none',lr_solver = 'lbfgs', lr_max_iter=1000):
        self.clf = LR(penalty=lr_penalty, max_iter=lr_max_iter, solver=lr_solver)
        self.ensemble = BOW_ensemble(self.clf,vectorizer)
